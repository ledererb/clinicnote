...
5. Készíts egy "orchestrator_summary" nevű mezőt, ami egy 3-4 mondatos orvosi és adatminőségi összefoglaló az egész vizitről és arról, ha bármilyen korrekciót vagy észrevételt tettél a feldolgozás során.`;
...
async function callClaudeText(system: string, userText: string, apiKey: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`OpenAI Text error: ${await res.text()}`);
...
async function callClaudeJson(system: string, userText: string, schema: any, apiKey: string): Promise<any> {
  ...
  if (!res.ok) throw new Error(`OpenAI JSON error: ${await res.text()}`);
...
export async function processAmbulansInternally(
  ...
  try {
    ...
    const [adminData, diagData, procData, anamnesisText, therapyData] = await Promise.all([
      callClaudeJson(ADMIN_PROMPT, userText, ADMIN_SCHEMA, apiKeys.openai),
      callClaudeJson(DIAGNOSES_PROMPT, userText, DIAGNOSIS_SCHEMA, apiKeys.openai),
      callClaudeJson(PROCEDURES_PROMPT, userText, PROCEDURE_SCHEMA, apiKeys.openai),
      callClaudeText(ANAMNEZIS_SYSTEM, userText, apiKeys.openai),
      callClaudeJson(THERAPY_SYSTEM, userText, THERAPY_SCHEMA, apiKeys.openai)
    ]);
...
`;
    ...
    const finalData = await callClaudeJson(ORCHESTRATOR_SYSTEM, orchestratorInput, ORCHESTRATOR_SCHEMA, apiKeys.openai);
